...
static const INT8U TXAddress[TX_ADR_WIDTH]={0x25,0x25,0x25,0x25,0x52};
static const INT8U RXAddress[TX_ADR_WIDTH]={0x25,0x25,0x25,0x25,0x52};
...
static void NRFReadContinul(INT8U Reg, INT8U *pRxBuff, INT16U Len)
{
    INT8U Transmit;
    INT16U ii;
...
static INT8U NRFRxMode(void)
{
    ...
    NRFReadContinul(NRF_READ_REG+RX_ADDR_P0, RxBuff, RX_ADR_WIDTH); //读出写入的地址
...
static INT8U NRFTxMode(void)
{
    ...
    NRFReadContinul(NRF_READ_REG+TX_ADDR,RxBuff,TX_ADR_WIDTH); //读出写入的地址
...
void NRFReceiveData(void)
{
    ...
    {
        NRFReadContinul(RD_RX_PLOAD, g_NRFCtrlMsg.RxBuff, RX_PLOAD_WIDTH);
